...
#### NavigationBar（通用顶部导航栏组件）
- **文件路径**：`entry/src/main/ets/components/NavigationBar.ets`
- **职责**：
  - 提供统一的顶部导航栏样式
  - 支持自定义标题、返回按钮文案
  - 支持右侧操作按钮（可选）
- **接口定义**：
  ...
  @Prop navBarBackgroundColor: string
...
#### InfoCard（通用信息卡片组件）
- **文件路径**：`entry/src/main/ets/components/InfoCard.ets`
- **职责**：
  - 展示标题、副标题、摘要
  - 展示等级标签与标签列表
  - 支持整卡点击事件
...
- **值符定位**：根据时干和局数
- **值使定位**：根据时支
- **格局识别**：三奇得使、天遁、地遁、人遁、伏吟、反吟、门迫、击刑、三奇应验等

### 4. 格局识别增强
#### 4.1 新增格局类型
- **门迫格局**：门克宫为门迫，主事阻滞、进展缓慢
- **击刑格局**：天干相刑（子刑卯、卯刑子等），主冲突、纠纷
- **三奇应验**：乙丙丁三奇得使，主吉祥、机遇

#### 4.2 格局可视化增强
- **视觉提醒系统**：凶格（门迫、击刑等）采用红色警示设计
- **颜色编码**：
  - 吉格：绿色系（#4CAF50）
  - 凶格：红色系（#FF4444）
  - 特殊格：橙色系（#FF9800）
...
```typescript
// 按宫位筛选格局
private filterPatternsByPalace(patterns: GejuPattern[], palaceIndex: number): GejuPattern[] {
  return patterns.filter(pattern => 
    pattern.palaceIndices.includes(palaceIndex)
  )
}

// 按重要性分级筛选
private filterPatternsByLevel(patterns: GejuPattern[], level: 'minor' | 'major' | 'special'): GejuPattern[] {
...
- **可测试性**：独立的引擎便于单元测试
- **可扩展性**：新增格局识别无需修改页面代码
- **可视化增强**：统一的视觉提醒系统提升用户体验
- **智能筛选**：按宫位和重要性级别灵活筛选格局
...
entry/src/main/ets/
├── components/           # UI可复用组件
│   ├── NavigationBar.ets        # 通用顶部导航栏组件
│   ├── InfoCard.ets             # 通用信息卡片组件
